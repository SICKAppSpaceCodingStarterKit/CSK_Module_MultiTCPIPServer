...
local function handleOnNewProcessing(object)
  ...
  Script.notifyEvent("MultiTCPIPServer_OnNewValueUpdate" .. multiTCPIPServerInstanceNumberString, multiTCPIPServerInstanceNumber, 'valueName', result, processingParams.selectedObject)
  ...
  --_G.logger:info(nameOfModule .. ": Processing on MultiTCPIPServer" .. multiTCPIPServerInstanceNumberString .. " was = " .. tostring(result))
  --Script.notifyEvent('MultiTCPIPServer_OnNewResult'.. multiTCPIPServerInstanceNumberString, true)

  --Script.notifyEvent("MultiTCPIPServer_OnNewValueToForward" .. multiTCPIPServerInstanceNumberString, 'MultiColorSelection_CustomEventName', 'content')
...
local function handleOnNewProcessingParameter(multiTCPIPServerNo, parameter, value, internalObjectNo)
  ...
  if multiTCPIPServerNo == multiTCPIPServerInstanceNumber then -- set parameter only in selected script
    ...
    if internalObjectNo then
      _G.logger:info(nameOfModule .. ": Update parameter '" .. parameter .. "' of multiTCPIPServerInstanceNo." .. tostring(multiTCPIPServerNo) .. " of internalObject No." .. tostring(internalObjectNo) .. " to value = " .. tostring(value))
